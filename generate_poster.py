...
def _ai_summarize_one(title: str, text: str, limit: int, api_key: str) -> str:
    ...
    prompt = (
        ...
        '【强制要求】摘要必须包含至少一个具体数据：\n'
        '  · 金额（罚款X万元、赔偿X元）、人数（涉及X人、X人死亡）、\n'
        '  · 时间（X月X日生效、X小时内）、数量（X起案件、X公斤）、\n'
        '  · 百分比（上升X%、占比X%）、地名（具体城市/区域）\n'
        '  · 如果原文有数据但你不提取，摘要不合格！\n'
        '【法律要点】涉及法院/检察院/仲裁/监管/合规/诉讼/合同/劳动的内容，必须点明法律结论（胜诉/败诉/违法/合规）。\n'
        '【禁止事项】\n'
        '  · 禁止套话开头（"据介绍""相关负责人表示""近年来"等）\n'
        '  · 禁止照搬标题，必须补充标题之外的新信息\n'
        '  · 禁止空话（"持续推进""有序开展""取得成效"等无实质内容）\n'
        f'【自检】输出前检查：(1)字数≤{limit}？(2)有具体数据吗？(3)以句号结尾吗？\n'
        '【合格示例】"北京市朝阳区人民法院判决，某公司因违法解除劳动合同，赔偿员工12万元。"（有地名+金额+法律结论）\n'
        '【不合格示例】"近年来，劳动争议案件持续增多，劳动者维权意识不断提升。"（无数据、无实质）\n\n'
        f'标题：{title}\n'
        f'内容：{text}\n\n'
