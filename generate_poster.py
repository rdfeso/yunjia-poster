...
def _has_valid_summary(summary: str, title: str = "") -> bool:
    ...
    # 6. 全文都是套话主题词（"消费者权益""高质量发展"）而无事实 → 假稿信号
    ...
    # 必须命中至少 2 个具体信息点（数据/地点/法律动作/机构/具量纲数字）
...
def _ai_summarize_one(title: str, text: str, limit: int, api_key: str) -> str:
    ...
    prompt = (
        ...
        '你是云嘉律师事务所每日资讯海报的新闻编辑。新闻六要素（5W1H）+ 至少2个具体数据点是底线。\n'

        f'【六要素硬性要求】摘要必须能让读者立刻还原事件：\n'

        '  · 何时（X月X日 / 近日 / 昨日 / 持续X天）\n'

        '  · 何地（具体到省/市/区/县，或机构/场所）\n'

        '  · 何人（机构名 / 嫌犯/车主/消费者/市民 等具体角色；不能光说"相关部门""有关方面"）\n'

        '  · 何事（具体动作/法律结论：判决/逮捕/通报/查处/退赔/兑付/罚款 等）\n'

        '  · 多少（金额X元/X万/人X名/死亡X人/涉及X起/比例X% — 必须有数字）\n'

        '  · 为何/如何（事件背景/原因/作案手法，1句即可）\n'

        f'【数据/事实硬性要求】必须包含至少2项【数字+地名+机构名+具体人名+法律动作】：\n'

        '  · 数字：金额（X万元/元）、人数（X人/X死亡）、时间（X月X日/X天）、数量（X起/X吨）、百分比（X%）\n'

        '  · 地名：具体到省/市/区/县，不要"华东地区""南方某省"这种含糊地理\n'

        '  · 机构/角色：某公司/某银行/某医院/某法院 等全名 + 嫌犯/车主/消费者 等具体人\n'

        '  · 法律动作：判决/逮捕/通报/查处/罚款/赔偿/退赔/兑付/吊销/撤销/驳回/认定/终审 等\n'

        '  · 没有具体数字的"金额较大""数量众多"是空话，直接判零分\n'

        f'【长度】摘要严格不超过{limit}字，结尾必须是完整句号（禁止用省略号截断）。\n'
        ...
        '  · 禁止用"消费者权益""高质量发展"这类主题词当事实\n'

        f'【自检】输出前必查：(1)字数≤{limit}？(2)有【至少2个数据/地名/机构/人名/法律动作】吗？(3)以句号结尾吗？(4)任何一句去掉会不会让事实塌掉？\n'

        '【合格示例】"滴滴陪诊杭州7月2日上线，2小时176元；行业存在黄牛加号收费3000元的灰色链条，已被浙江消保委约谈。"\n'

        '【不合格示例】"消费者权益典型案例发布，霸王条款被集中曝光。"（零数据零地点零机构，零分）\n'

        '【不合格示例】"首届北京—伯明翰科技周举办，150余位代表参会，聚焦人工智能领域。"（只有1个数字，零机构零法律动作）\n'

        '【重要】如果新闻内容没有具体数据/地点/机构/人名/法律动作，或者只有套话主题词，请直接输出"无实质信息"四个字，不要强行写摘要。\n\n'
